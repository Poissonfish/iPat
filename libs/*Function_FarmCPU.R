...
`FarmCPU` <-
  function(Y=NULL,GD=NULL,GM=NULL,CV=NULL,GP=NULL,Yt=NULL,DPP=1000000,kinship.algorithm="FARM-CPU",file.output=TRUE,cutOff=0.01,method.GLM="FarmCPU.LM",method.sub="reward",method.sub.final="reward",method.bin="static",bin.size=c(5e5,5e6,5e7),bin.selection=seq(10,100,10),
    ...
    for (trait in 2: ncol(Y))  {
      ...
      #print(head(CV1))
      if(length(seqTaxa)<1) stop("GAPIT stoped as no data in Y")
...
`FarmCPU.P.Threshold` <-
  function(GD=NULL,GM=NULL,Y=NULL,trait="",theRep=100){
    #Input: myGD - Genotype
    #Input: myGM - SNP name, chromosome and BP
    #Input: myY - phenotype, 2 columns
    ...
    #theRep=theRep
    #trait=trait
    if(is.null(myGD))return(NULL)
    if(is.null(myGM))return(NULL)
    if(is.null(myY))return(NULL)
    set.seed(12345)
    i=1
    for(i in 1:theRep){
      index=1:nrow(myY)
