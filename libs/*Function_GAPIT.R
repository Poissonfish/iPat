...
`GAPIT.ID` <-
  function(DP=NULL,IC=NULL,SS=NULL,RS=NULL,cutOff=0.01,
           ...
           file.output=TRUE,SNP.MAF=0,CG=CG){
    ...
    if(is.null(DP)&is.null(IC))#inputdata is other method result
    {
      ...
      if(!is.null(IC$GD)&DP$SNP.test)
      {
        ...
        if(!is.null(PWI.Filtered))
        {
          ...
          print("Manhattan plot (Genomewise)..." )
          if(DP$file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps), name.of.trait = DP$name.of.trait, DPP=DP$DPP, plot.type = "Genomewise",cutOff=DP$cutOff,seqQTN=DP$QTN.position,plot.style=DP$plot.style) 
          
          print("Manhattan plot (Chromosomewise)..." )
          
          #if(file.output) GAPIT.Manhattan(GI.MP = PWIP$PWIP[,2:4], name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
          print(dim(IC$GD))
          if(DP$file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps),GD=IC$GD[,-1], CG=DP$CG,name.of.trait = DP$name.of.trait, DPP=DP$DPP, plot.type = "Chromosomewise",cutOff=DP$cutOff)
    ...
    if (SUPER_GS)
    {
      ...
      if(hasGenotype &SNP.test &!is.na(Compression[1,4]))     #require not NA REML 
      {
        ...
        if(!is.null(PWI.Filtered))
        {
          ...
          #if(file.output) GAPIT.Manhattan(GI.MP = PWIP$PWIP[,2:4], name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
          if(file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps),GD=GD,CG=CG, name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
...
`GAPIT.Manhattan` <-
  function(GI.MP = NULL,GD=NULL,name.of.trait = "Trait",plot.type = "Genomewise",
           DPP=50000,cutOff=0.01,band=5,seqQTN=NULL,plot.style="Oceanic",CG=NULL){
    ...
    if(plot.type == "Chromosomewise"&!is.null(GD))
    {
      ...
      for(i in 1:numCHR)
      {
        ...
        #print(dim(X))
        subGD=X[,subset[,5]]
        if(is.null(CG))candidata=subset[subset[,3]==max(subset[,3]),5]
        #if(!is.null(CG)&length(CG)!=numCHR)candidata=NULL		
        if(!is.null(CG))candidata=CG[i]
        if(!is.na(candidata))
        {
          print(paste("choosed candidata gene is ",i," chromosome ",candidata,sep="") )
          r2_storage=NULL
          for (j in 1:ncol(subGD))
          {
            r2=cor(X[,candidata],subGD[,j])^2
            r2_storage=append(r2_storage,r2)
          }
          r2_storage[is.na(r2_storage)]=0
          r2_color=cbind(r2_storage,NA)
          r2_color[r2_color[,1]<=0.2,2]="darkblue"
          r2_color[0.2<r2_color[,1]&r2_color[,1]<=0.4,2]="cyan"
          r2_color[0.4<r2_color[,1]&r2_color[,1]<=0.6,2]="green3"
          r2_color[0.6<r2_color[,1]&r2_color[,1]<=1,2]="brown1"
        }else{
          r2_color=cbind(0,"darkblue")
        }#end !is.null(candidata)
        r2_color[subset[,5]==candidata,2]="white"
        #print(dim(r2_color))
        #if(i==18)print(r2_color)
        #print(dim(subset))
        ...
        plot(y~x,type="p", ylim=c(0,y.lim), xlim = c(min(x), max(x)),
             col = "black", xlab = expression(Base~Pairs~(x10^-6)), 
        ...
        par(mar=c(0,0,0,0))
        par(mar=c(1,18,4,2),cex=0.5)
        lengendata=matrix(0.5,5,1)
        #lengendata[,2]=0.5
        barplot(lengendata,col=provcol,horiz=F,axes=F,border ="black" ,main=expression(R^"2"))
        axis(4,seq(0,2.5,by=0.5),seq(0,1,by=0.2))
  ...
  function(DP=NULL,IC=NULL){
    ...
    if(DP$SNP.test&DP$kinship.algorithm!="FarmCPU"&DP$kinship.algorithm!="MLMM"){
      ...
      gapitMain <- GAPIT.Main(Y=ic_Y,GD=ic_GD[,-1],GM=ic_GM,KI=ic_KI,Z=Z,CV=DP$CV,CV.Inheritance=DP$CV.Inheritance,GP=DP$GP,GK=DP$GK,SNP.P3D=DP$SNP.P3D,kinship.algorithm=DP$kinship.algorithm,
                              ...
                              QTN=DP$QTN, QTN.round=DP$QTN.round,QTN.limit=DP$QTN.limit, QTN.update=QTN.update, QTN.method=DP$QTN.method, Major.allele.zero=DP$Major.allele.zero,
...
`GAPIT3` <-
  function(Y=NULL,G=NULL,GD=NULL,GM=NULL,K=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
           ...
           QTN=NULL, QTN.round=1,QTN.limit=0, QTN.update=TRUE, QTN.method="Penalty", Major.allele.zero = FALSE,
           method.GLM="fast.lm",method.sub="reward",method.sub.final="reward",method.bin="static",output.numerical = FALSE,
    ...
    if(!is.null(Y)){
      ...
      for (trait in 2: ncol(Y))  {
        ...
        DP=GAPIT.DP(Y=Y[,c(1,trait)],G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance= CV.Inheritance,GP=GP,GK=GK,
                    ...
                    kinship.cluster=CMLM$kinship.cluster, kinship.group=CMLM$kinship.group,kinship.algorithm= kinship.algorithm,                                                    
