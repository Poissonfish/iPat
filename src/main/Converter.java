...
class iPat_converter{
	final int sub_n = 512, sub_m = 16384;
	String sep = "\t";
	...
	public iPat_converter (String input, String output, String gd, String gm) throws IOException {
		...
		switch (input) {
			case "num":
				InputFormat = format.Numerical; 
				break;
			case "hmp":
				InputFormat = format.Hapmap; 
				break;
			case "vcf":
				InputFormat = format.VCF; 
				break;
			case "plink":
				InputFormat = format.PLINK; 
				break;
		}
		switch (output) {
			case "num":
				OutputFormat = format.Numerical;
				break;
			case "hmp":
				OutputFormat = format.Hapmap;
				break;
			case "vcf":
				OutputFormat = format.VCF;
				break;
			case "plink":
				OutputFormat = format.PLINK;
				break;
		}	
		switch (InputFormat) {
			case Numerical:
				NumToPlink(GD_path, GM_path); 
				break;
			case Hapmap:
				switch (OutputFormat) {  
					case Numerical: 
	...
	private void message() {
		System.out.println("Usage: java -jar ipat_converter.jar [input-format] [output-format] [path to data]");
		System.out.println("");
		System.out.println("Option for input and output format:");
		System.out.println("	—-num		Numeric format, individuals listed as rows");
		System.out.println("	—-hmp		Hapmap ");
		System.out.println("	—-vcf		VCF");
		System.out.println("	—-plink 	PLINK format (.ped and .map)");
		System.out.println("Options for path to data:");
		System.out.println("	-GD Path to input genotype data ");
		System.out.println("	-GM Path to input map data (Only required for num and plink)");
		System.out.println("");
		System.out.println("Example 1: To convert files from numeric to hapmap:");
		System.out.println("	java -jar ipat_converter.jar —-num —-hmp —GD data.dat —GM data.map");
		System.out.println("Example 2: To convert files from vcf to plink:");
		System.out.println("	java -jar ipat_converter.jar —-vcf —-plink —GD data.vcf");	
	}
