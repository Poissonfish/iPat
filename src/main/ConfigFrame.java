...
public class ConfigFrame extends JFrame implements ActionListener{
	...
	public static method[]  ListGWAS = {method.GAPIT, method.FarmCPU, method.PLINK}, 
							...
							analysis_method = {method.NA, method.NA};
	...
	public ConfigFrame(int MOindex, analysis Analysis, iPatPanel.FORMAT format, Findex[] file_index) throws IOException{
		...
		// Top (Common) pane
			...
			// WD panel initialization
				...
				project_name.longfield.setText("Project_"+ MOindex);
		...
		this.addMouseListener(new MouseAdapter(){
			...
			public void mousePressed(MouseEvent e){
				pt = e.getPoint();
				for(method index : (Analysis == analysis.GWAS? ListGWAS:ListGS)){
					if(label_method[index.index].getBounds().contains(pt)){
						indexDrag = index; 
						tempLabel = label_method[index.index].getLocation();
						break;
					}
				}	
			...
			public void mouseReleased(MouseEvent e){
				if(indexDrag != method.NA){
					if(pane_config.getBounds().contains(e.getPoint())){
						pane_config.MethodDrop(indexDrag);
					};
		...
		this.addMouseMotionListener(new MouseMotionAdapter(){
			...
			public void mouseDragged(MouseEvent e){
				Point pt_drag = e.getPoint();
                int dx = e.getX() - pt.x;
                int dy = e.getY() - pt.y;
                // entering panel with a method
				if(indexDrag != method.NA && pane_config.getBounds().contains(pt_drag)){
					pane_config.MethodClear();
					pane_config.MethodEnter();
					label_method[indexDrag.index].setLocation(tempLabel.x + dx, tempLabel.y + dy);	
				// drag after methods deployed
				}else if(indexDrag != method.NA && pane_config.isDeployed){
					pane_config.MethodDrop(pane_config.existmethod);
					label_method[indexDrag.index].setLocation(tempLabel.x + dx, tempLabel.y + dy);	
				// drag before methods deployed
				}else if(indexDrag != method.NA){
					pane_config.MethodAbsent();
					label_method[indexDrag.index].setLocation(tempLabel.x + dx, tempLabel.y + dy);	
				}
				repaint();
            }
			...
			public void windowClosing(WindowEvent e) {
				...
				if(pane_config.isDeployed){
					...
					case GWAS:
						...
						iPatPanel.isDeployed[MOindex][analysis.GWAS.index] = true;
						analysis_method[analysis.GWAS.index] = pane_config.existmethod;
						iPatPanel.popup_gwas.setText("GWAS ("+ pane_config.existmethod.Name() +")");
					...
					case GS:
						...
						iPatPanel.isDeployed[MOindex][analysis.GS.index] = true;
						analysis_method[analysis.GS.index] = pane_config.existmethod;	
						iPatPanel.popup_gs.setText("GS ("+ pane_config.existmethod.Name() +")");
						for (int i = 0; i < iPatPanel.command_gs[MOindex].length; i++){
							System.out.println(iPatPanel.command_gs[MOindex][i]);		
						}
						break;
					}
					iPatPanel.popup_run.setEnabled(true);
				}
				pane_config.save();
			}
		});
...
class Configuration extends JFrame implements ActionListener, WindowListener{
	String  P_name = "NULL", G_name = "NULL", GD_name = "NULL", GM_name = "NULL",
			C_name = "", K_name = "",
			VCF_name = "", PED_name = "", MAP_name = "",
			BED_name = "", FAM_name = "", BIM_name = "", 
			R_exe = "";
	public static int C_provided = 0, K_provided = 0;
    int MOindex = 0;
    String file_format = "";
	///////////////////////////////////////////////////////////////////////////////////////
	Preferences pref;
	...
	public JPanel config_gapit() throws IOException{
		...
		Project_g.longfield.setText("Project_"+ MOindex);
	...
	public JPanel config_farm() throws IOException{
		...
		Project_f.longfield.setText("Project_"+ MOindex);
	...
	public JPanel config_plink(){
		...
		Project_p.longfield.setText("Project_"+ MOindex);
	...
	public JPanel config_rrblup() throws IOException{
		...
		Project_r.longfield.setText("Project_"+ MOindex);
	...
	public JPanel config_BGLR() throws IOException{
		...
		Project_b.longfield.setText("Project_"+ MOindex);
	...
	String[] run_BGLR(Findex[] file_index){
		...
		if(C_provided != 0){
			...
			for (int i = 0; i < CO_names_b.length; i++){CO_model = CO_model + panel_covariates_b.getSelected() + "sep";}
		}else{
			C = "NA";
		}
	...
	public JPanel config_BSA() throws IOException{
		...
		Project_bsa.longfield.setText("Project_"+ MOindex);
